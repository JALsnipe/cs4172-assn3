...
public class DragObject : MonoBehaviour {
	...
	void Update () {
		...
		foreach (Touch touch in Input.touches) {
			...
			float dist = 0.0f;
			//Intersects ray with the plane. Sets dist to distance along the ray where
			//intersects
				targetPlane.Raycast(ray, out dist);
			//Returns point dist along the ray.
			Vector3 planePoint = ray.GetPoint(dist);
			Debug.Log("Point=" + planePoint);
			//True if finger touch began. If ray intersects collider,
			//set pickedObject to transform of collider object
